...
## Non-React hosts — web component

For plain HTML / Angular / Vue and other non-React hosts, use the companion package
[`@project-sunbird/generic-editor-v2-web-component`](../web-component) which ships a
self-contained `<sunbird-generic-editor>` custom element (React bundled in):

```html
<script src=".../sunbird-generic-editor-web-component.js"></script>
<sunbird-generic-editor id="editor"></sunbird-generic-editor>
<script>
  const el = document.getElementById('editor');
  el.context = { uid, sid, did, channel, pdata, user, framework };
  el.config = { language: 'en' };          // largeUpload, previewConfig, … optional
  el.contentId = 'do_123';                  // omit for a new upload
  el.addEventListener('editorClose', () => history.back());
</script>
```

See the [web-component README](../web-component/README.md) for the full property/event API.
